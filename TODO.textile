...
- travis-ci: fix the stats page and write tests for that code
- rabbitmq message rate stats

- clean up stalled jobs from the db

- travis-hub: should read the db config from the (heroku) env var if present

- travis-worker: NoMethodError: undefined method `deep_symbolize_keys' for #<Array:0x50b70f41>
      initialize at /home/travis/.rvm/gems/jruby-head/gems/hashr-0.0.17/lib/hashr.rb:39
...
- travis-core: all config jobs get pushed to builds.common. is that what we want?
