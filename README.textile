...
* An "AMQP":http://bit.ly/amqp-model-explained worker for running a project's test suite remotely.
* A websocket server (we use "Pusher":http://pusher.com) for tailing build results to the browser.
* An AMQP application that collects build reports, workers state, propagates them to Pusher, delivers notifications and so on.
* Tools that provision CI environment (to provide databases, RabbitMQ, Rubies/JDK/Node.js versions do so on) and build VM images.
