...
class IrcNotificationsTest < NotificationsTestCase
  ...
  def test_one_irc_notification
    ...
    assert_match /\[Travis-CI\] svenfuchs\/minimal#1 \(master - 62aae5f\): the build has failed/, @irc_mock.output[1]
    assert_match /\[Travis-CI\] GitHub changeset : https:\/\/github.com\/foo\/bar-baz\/compare\/master\.\.\.develop/, @irc_mock.output[2]
    assert_match /\[Travis-CI\] Full build details : http:\/\/test.travis-ci.org\/svenfuchs\/minimal\/builds\/1/, @irc_mock.output[3]
  end
  ...
  def test_two_irc_notifications
    ...
    assert_match /\[Travis-CI\] svenfuchs\/minimal#1 \(master - 62aae5f\): the build has failed/, @irc_mock.output[7]
    assert_match /\[Travis-CI\] GitHub changeset : https:\/\/github.com\/foo\/bar-baz\/compare\/master\.\.\.develop/, @irc_mock.output[8]
    assert_match /\[Travis-CI\] Full build details : http:\/\/test.travis-ci.org\/svenfuchs\/minimal\/builds\/1/, @irc_mock.output[9]
  end
