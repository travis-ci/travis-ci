  ...
  describe('the fold pattern', function() {
    describe('for bundle install', function() {
      it('matches with single line breaks', function() {
        ...
        expect(log).toMatch(Travis.Log.FOLDS['bundle']);
      });
    ...
    describe('for rake db:migrate', function() {
      it('matches with bundle exec prepended', function() {
        ...
        expect(log).toMatch(Travis.Log.FOLDS['migrate']);
      });
    ...
    describe('for rake schema:load', function() {
      it('matches with bundle exec prepended', function() {
        ...
        expect(log).toMatch(Travis.Log.FOLDS['schema']);
      });
    ...
    describe('for executing ruby lines', function() {
      it('matches lines like output by rake', function() {
        var log = '/home/vagrant/.rvm/rubies/ree-1.8.7-2011.03/bin/ruby -S' +
                  'bundle exec rspec ./spec/controllers/aspect_memberships_controller_spec.rb' +
                  './spec/controllers/activity_streams/photos_controller_spec.rb';

        expect(log).toMatch(Travis.Log.FOLDS['exec']);
      });
      ...
      it('does not match other lines starting with a path to the rvm ruby dir', function() {
        ...
        expect(log).not.toMatch(Travis.Log.FOLDS['exec']);
      });
    });
  });
});
